...
export default function TripPlanner({ groupId, apiBase = '', getToken, demo = false, onItinerarySaved, request }) {
  ...
  async function api(path, method = 'GET', body, identity = user) {
    if (request) return request(path, method, body);
  ...
  }, [groupId, apiBase, demo, getToken, request]);
